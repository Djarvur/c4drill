...
*Default types by nesting level:* ++|++ Level ++|++ Parent ++|++ Default
Type ++|++ ++|++-------++|++--------++|++--------------++|++ ++|++ C1
++|++ None (root) ++|++ `system` ++|++ ++|++ C2 ++|++ system,
systemExternal, box ++|++ `container` ++|++ ++|++ C3 ++|++ container
++|++ `component` ++|++
...
*Default type by parent* (`defaultTypeForParent`): ++|++ Parent type
++|++ Inferred child type ++|++ Level ++|++
++|++-------------++|++---------------------++|++-------++|++ ++|++
(none — root) ++|++ `system` ++|++ C1 ++|++ ++|++ `system` ++|++
`container` ++|++ C2 ++|++ ++|++ `box` ++|++ `system` ++|++ C1
(same-level grouping) ++|++ ++|++ `container` ++|++ `component` ++|++ C3
++|++ ++|++ `containerBox` ++|++ `container` ++|++ C2 (same-level
grouping) ++|++ ++|++ `componentBox` ++|++ `component` ++|++ C3
(same-level grouping) ++|++ ++|++ (other: db, queue, etc.) ++|++
`system` ++|++ C1 fallback ++|++
...
++|++ ++|++ `system` or `containerBox` ++|++ `containerDb` ++|++
`containerQueue` ++|++ C2 ++|++ ++|++ `container` or `componentBox`
++|++ `componentDb` ++|++ `componentQueue` ++|++ C3 ++|++
...
`++[[++use++]]++` location), not the template's lexical location
(XC-03).
...
|`peer` |`"unit++_++name"` |*Required:* Target unit identifier

|`arrow` |`forward` (default), `reverse`, `bidirectional`, `none` |Arrow
direction
...
|`labelPosition` |`middle` (default), `tail`, `head` |Where label
appears
|===

[[templates-template--use]]
=== Templates (++[++template.++*]++ {plus} ++[[++use++]]++)

Define a parametrized unit template once and instantiate it N times with
distinct parameter values. A `++[++template.++<++name++>]++` table
declares its parameters and the unit shape (fields, links, subunit
subtrees); each `++[[++use++]]++` directive instantiates it under a
parent with concrete parameter values.
...
|===
|Field |Description
|`params = ++[++"a", "b", ...++]++` |Required: declares the named
parameters. ALL are required on every `++[[++use++]]++` (no defaults).

|`name`, `description`, `technology`, `reference`, `color` |Standard
unit fields; `$++{++param}` substitutes into each.

|`type` |Standard unit type. Must be valid for the instantiation parent.

|`++[[++template.++<++name++>++.link++]]++` |Fixed link set (TMPL-03);
peer/description/technology fields accept `$++{++param}`.

|`++[++template.++<++name++>++.++<++child++>]++` |Subunit subtree
(TMPL-04); child key verbatim (D-04), field values substituted.
|===
...
|===
|Field |Description
|`template = "++<++name++>++"` |Required: the
`++[++template.++<++name++>]++` to instantiate.

|`parent = "++<++dotted.path++>++"` |Optional: placement path
(empty/omitted = top-level). Produced path =
`parent {plus} "." {plus} name-param`.

|`name = "++<++value++>++"` |Required: fills the produced unit's last
path segment AND the template's `$++{++name}` token.

|(other keys) |The template's remaining params; each must match a
...
produce N links, not a fan-out (no `for++_++each`).
...
direct links (validator rule — split into a leaf template {plus} a
...
|===
|Field |Description
|`path = "++<++relative++>++"` |Required: TOML file to include; relative
to the including file's directory (INC-02).

|`once = true` |Optional: opts into visited-set dedup (INC-06) — a file
included again via any path is skipped.
|===
...
|Sibling (D-13) |`platform.api` |`cache` |`platform.cache` (sibling
under nearest ancestor)

|Aunt (D-14) |`platform.api.handlers.auth` |`queue` |`platform.queue`
(walk up past empty scopes)

|Root (D-15) |`platform.api.handlers.auth` |`messageBus` |`messageBus`
(top-level, walk reaches root)

|Absolute (D-16) |any |`platform.cache` |`platform.cache` (dot = no
walk-up)
|===
...
[arabic]
. *link:examples/01-minimal.toml[01-minimal.toml]* - Minimal working
example (person {plus} system {plus} single link)
. *link:examples/02-nested.toml[02-nested.toml]* - Nested structure
(C2/C3 levels)
. *link:examples/03-links.toml[03-links.toml]* - Link syntax (all
attributes)
. *link:examples/04-styling.toml[04-styling.toml]* - Visual
customization (colors, borders, edges)
. *link:examples/05-ecommerce.toml[05-ecommerce.toml]* - Realistic full
architecture
. *link:examples/06-templates.toml[06-templates.toml]* - Templates
(`++[++template.++*]++` define {plus} `++[[++use++]]++` instantiate,
TMPL-01..10)
. *link:examples/07-relative-peer.toml[07-relative-peer.toml]* -
Relative peer walk-up (4 cases: sibling, aunt, root, absolute)
. *link:examples/08-include/[08-include/]* - Multi-file composition
(`++[[++include++]]++`, `once`, cross-file subunits)
. *link:examples/09-composed/[09-composed/]* - All four features
...
[arabic]
. *Always include `++[++properties++]++` with `name`* - Required field
. *Always include `type` and `name` for each unit* - Required fields
. *Use descriptive section names* - They become identifiers (e.g.,
`user++_++service` not `svc1`)
. *Use dotted notation for nesting* - `parent.child` pattern
. *Prefer `++[[++link++]]++` over `++[[++linkFrom++]]++`* - More natural
(source → target)
. *Include `peer` field in every link* - Required field specifies target
unit
. *Include `technology` for systems/databases* - Shows tech stack in
diagram
. *Use `external` suffix for external units* - `systemExternal`,
`dbExternal`
. *Validate before committing* - Run `c4drill ++<++file.toml++>++` to
check
...
|`-f html` |`html` |Safari/WebKit (which ignores SVG `++<++a++>++`
links); also `file://` viewing
...
_Skill version: 1.2.0_ _C4Drill version: v1.10{plus}_
