...
- **Unit templates — parametrized unit definitions** (`tooling`) — `.planning/todos/pending/2026-08-08-unit-templates-parametrized-definitions.md`. Define an almost-complete unit with parametrized fields, instantiate with params to produce concrete units. Two references: go-metadot macros (reference = `metadot.pl`; the Go port does NOT implement macros) and PlantUML `!procedure` (named args + trailing defaults — the better fit for TOML). Has OPEN DESIGN QUESTIONS: where substitution happens (text-preprocess vs structured post-parse vs text/template engine — recommendation: structured post-parse), named vs positional params, instantiation syntax. Resolve design before implementation. Depends on relative-peer todo for clean link handling.
- **Include directive — multi-file diagrams** (`tooling`) — `.planning/todos/pending/2026-08-08-include-directive-multi-file-diagrams.md`. Assemble a diagram from multiple input TOML files (split large models; isolate template libraries). References: go-metadot `include` (`metadot.pl:839`, cycle detection via `@incChain`) and PlantUML `!include`/`!include_once`. Has OPEN DESIGN QUESTIONS: pre-parse byte-concat vs parse-then-merge structs (recommendation: parse-then-merge), directive syntax, merge conflict rules. Must resolve FIRST in the pipeline (`include → template-expand → relative-peer-resolve → validate`).
