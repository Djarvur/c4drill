# Requirements Archive: v1.15 Hierarchy Wrapping and Granular Keys
...
**Outcome notes (added at milestone close, 2026-08-31):**
- LBL-01 — shipped as specified (all labels suppressed), then NARROWED post-release by quick task 260831-01u: `--no-labels` now suppresses edge labels only, keeping node/cluster labels and the legend. Original all-label semantics reversed after user review; goldens, KEY-03 matrix and docs re-baselined.
- WRAP-01/WRAP-03 — validated with one documented correction: the post-release root-diagram flood was bisected to v1.21.0 CTX-02/03 whole-subtree unfolding (not the v1.22.0 WRAP change); fixed and pinned by the deepcross fixture invariant.
