...
* Also speaks `textDocument/completion` (all 17 unit types with nesting
inference and generic-type promotion, unit/link fields, enum values,
peers, template params, include paths), `hover` (resolved peer paths,
level and promoted type; template param info), `definition` (peer to
unit section, template reference, include target), `documentSymbol`
(unit outline) and `textDocument/formatting` (fmt parity for both
formats).
...
click-through drill-down navigation and breadcrumbs. The command and
settings surface is listed in `extension/package.json`.
...
chat panel whose edit proposals land as confirmed diffs. The same UI
is also available in a browser via `./gui --serve`. Build, run and
configuration details in `gui/README.md`.
