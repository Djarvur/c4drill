...
A minimal graph showing rank hints. The per-link `rank` attribute puts
the link's endpoints on the same rank (`rank = "equal"`) or orders them
along the layout direction (`rank = "forward"` / `"reverse"`).
`D` carries a link to `C` to satisfy the orphan rule (see
<<Validation Rules>>).
