...
field overrides the global value for that unit's own diagram.
...
Flags:
  ...
  -h, --help                help for c4drill
      ...
      --no-labels           Suppress all element label text: shape-only nodes/edges/clusters so layout re-flows (legend stays)
...
* `--plain` (v1.21) renders with author-custom formatting *ignored* —
  ...
  ** `properties.edges` — default spline routing.
...
* *Granular suppression switches* (v1.22) turn off one formatting
  ...
  ** `--no-labels` — all *element* label text is suppressed at the
     graph layer: nodes render as bare shapes, edges and clusters
     (wrapper and boundary included) carry no label, so dot lays out
     without label geometry. Colour/style semantics, cluster
     structure, and explore/reference URL attributes survive. The 🔍
     and 📖 glyphs are label content and go too. Pinned boundaries:
